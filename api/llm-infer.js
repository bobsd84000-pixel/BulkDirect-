export async function POST(req) {
  ...
  const zaiResult = await tryZAI(prompt, max_tokens, temperature);
  if (zaiResult.success) {
    return new Response(
      JSON.stringify({ provider: 'z-ai', result: zaiResult.data }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  }
...
async function tryZAI(prompt, maxTokens, temperature) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.text();
      console.error(`Z.ai error (${response.status}):`, error);
      return { success: false, error: `HTTP ${response.status}` };
    }
